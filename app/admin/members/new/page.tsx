...
export default function Page(){
  ...
  useEffect(()=>{(async()=>{
    const r=await fetch('/api/admin/members/table?page=1', {cache:'no-store'});
    const d=await r.json();
    if(!r.ok) return setMsg(d.message||'โหลดไม่สำเร็จ');
    setRows((d.rows||[]).filter((x:Row)=>x.status==='active').slice(0,20));
  })();},[]);

  return <main className="min-h-screen bg-[#0d1020] text-white p-6 md:p-10"><div className="mx-auto max-w-6xl space-y-4">
    <h1 className="text-3xl font-bold text-yellow-300">สมาชิกใหม่</h1>
    {msg && <p className="text-yellow-300">{msg}</p>}
    <div className="overflow-x-auto rounded-2xl border border-zinc-700 bg-zinc-900/70">
      <table className="min-w-full text-sm"><thead><tr>{['ID','Username','ชื่อ','เบอร์','สถานะ','สมัครเมื่อ'].map(h=><th key={h} className="bg-zinc-800 px-3 py-2 text-left text-zinc-300">{h}</th>)}</tr></thead>
